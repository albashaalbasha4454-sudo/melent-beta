...
export const UserSection: React.FC<UserSectionProps> = ({ type }) => {
  ...
  const columns = [
    { header: type === 'Clients' ? t('client') : t('supplier'), accessor: (u: Client) => (
    ...
    { header: t('location'), accessor: (u: Client) => (
      <div className="flex items-center gap-2">
        <MapPin size={14} className="text-slate-300" />
        <span>{u.location}</span>
      </div>
    )},
    { header: t('contact_person'), accessor: (u: Client) => (
      <div className="flex items-center gap-2">
        <Building2 size={14} className="text-slate-300" />
        <span>{u.contactPerson}</span>
      </div>
    )},
    { header: t('contact'), accessor: (u: Client) => (
      <div className="flex items-center gap-2">
        <Phone size={14} className="text-slate-300" />
        <span dir="ltr">{u.phone || t('not_available')}</span>
      </div>
    )},
  ];
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h2 className="text-3xl font-black text-brand-navy tracking-tight uppercase">
            {type === 'Clients' ? t('manage_clients') : t('manage_suppliers')}
          </h2>
          <p className="text-[10px] font-black text-brand-green uppercase tracking-[0.3em] mt-1">
            {type === 'Clients' ? t('clients_network') : t('suppliers_network')}
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button 
            ...
            className="bg-brand-navy text-white px-6 py-4 rounded-2xl font-black text-sm shadow-xl shadow-brand-navy/10 hover:bg-brand-green transition-all flex items-center gap-3 group whitespace-nowrap"
          >
            <Plus size={18} className="text-brand-cyan group-hover:rotate-90 transition-transform" />
            {type === 'Clients' ? t('add_client') : t('add_supplier')}
          </button>
        </div>
      </div>
