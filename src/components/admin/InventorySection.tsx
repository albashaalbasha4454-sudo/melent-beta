...
export const InventorySection: React.FC = () => {
  ...
  const columns = [
    { header: t('products'), accessor: (p: Product) => (
    ...
    <div className={`animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8 ${isRTL ? 'text-right' : 'text-left'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100">
           <div className={`w-12 h-12 bg-brand-navy rounded-2xl flex items-center justify-center text-white mb-4 ${isRTL ? 'mr-0' : 'ml-0'}`}>
              <Package size={24} />
           </div>
           <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">{t('total_pieces')}</p>
           <p className="text-2xl font-black text-brand-navy tracking-tighter">
             {products.reduce((acc, p) => acc + p.stock, 0).toLocaleString()}
           </p>
        </div>
        <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100">
           <div className={`w-12 h-12 bg-red-50 rounded-2xl flex items-center justify-center text-red-500 mb-4 ${isRTL ? 'mr-0' : 'ml-0'}`}>
              <AlertCircle size={24} />
           </div>
           <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">{t('shortage_alerts')}</p>
           <p className="text-2xl font-black text-red-500 tracking-tighter">
             {lowStock.length} {t('products')}
           </p>
        </div>
      </div>
