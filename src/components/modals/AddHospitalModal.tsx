...
export const AddHospitalModal: React.FC<AddHospitalModalProps> = ({ isOpen, onClose, onAdd, hospitalToEdit }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.location) {
      setError(t('hospital_data_error'));
      return;
    }
          ...
          <div className={`flex items-center gap-4 ${isRTL ? 'flex-row-reverse text-right' : 'text-left'}`} dir={isRTL ? 'rtl' : 'ltr'}>
            ...
            <div>
              <h3 className="text-2xl font-black text-brand-navy leading-none">
                {hospitalToEdit ? t('edit_hospital_details') : t('add_new_partner_hospital')}
              </h3>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1 leading-none">{t('healthcare_partner_desc')}</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className={`p-10 space-y-8 ${isRTL ? 'text-right' : 'text-left'}`} dir={isRTL ? 'rtl' : 'ltr'}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-1.5 md:col-span-2">
              <label className="text-[10px] font-black uppercase text-slate-400 mx-2 tracking-widest">{t('hospital_center_name')}</label>
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase text-slate-400 mx-2 tracking-widest">{t('location_country_city')}</label>
              <div className="relative">
                <MapPin className={`absolute ${isRTL ? 'right-4' : 'left-4'} top-1/2 -translate-y-1/2 text-slate-300`} size={18} />
                <input type="text" value={formData.location || ''} onChange={e => setFormData({...formData, location: e.target.value})} className={`w-full bg-slate-50 border-2 border-transparent rounded-2xl p-4 ${isRTL ? 'pr-12 pl-4' : 'pl-12 pr-4'} focus:bg-white focus:border-brand-navy/10 transition-all font-bold text-sm`} placeholder="Location" />
              </div>
            </div>

            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase text-slate-400 mx-2 tracking-widest">{t('partnership_status')}</label>
              <select value={formData.contractStatus || 'Active'} onChange={e => setFormData({...formData, contractStatus: e.target.value as any})} className="w-full bg-slate-50 border-2 border-transparent rounded-2xl p-4 focus:bg-white focus:border-brand-navy/10 transition-all font-bold text-sm">
                ...
                <option value="Under Negotiation">{t('under_negotiation')}</option>
                <option value="Expired">{t('expired_status')}</option>
              </select>
            </div>

              <div className="grid grid-cols-2 gap-4">
               <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase text-slate-400 mx-2 tracking-widest">{t('rating_label')}</label>
                <div className="relative">
                  <Star className={`absolute ${isRTL ? 'right-4' : 'left-4'} top-1/2 -translate-y-1/2 text-brand-gold`} size={18} />
                  <input type="number" step="0.1" max="5" value={formData.rating || 0} onChange={e => setFormData({...formData, rating: Number(e.target.value)})} className="w-full bg-slate-50 border-2 border-transparent rounded-2xl p-4 focus:bg-white focus:border-brand-navy/10 transition-all font-black text-sm text-center tabular-nums" />
                </div>
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase text-slate-400 mx-2 tracking-widest">{t('primary_contact_details')}</label>
            ...
            <div className="space-y-1.5 md:col-span-2">
              <label className="text-[10px] font-black uppercase text-slate-400 mx-2 tracking-widest">{t('medical_specialties')}</label>
              <div className={`flex gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                <input type="text" value={specialtyInput} onChange={e => setSpecialtyInput(e.target.value)} onKeyPress={e => e.key === 'Enter' && (e.preventDefault(), addSpecialty())} className="flex-1 bg-slate-50 border-2 border-transparent rounded-2xl p-4 focus:bg-white focus:border-brand-navy/10 transition-all font-bold text-sm" placeholder={t('add_specialty_placeholder')} />
                <button type="button" onClick={addSpecialty} className="px-6 bg-brand-navy text-white rounded-2xl font-black text-xs uppercase tracking-widest">{t('add_button_text')}</button>
              </div>
          ...
          <div className={`pt-6 flex gap-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
             <button type="submit" className="flex-1 bg-brand-navy text-white py-5 rounded-[1.5rem] font-black text-sm shadow-2xl shadow-brand-navy/20 hover:bg-brand-green transition-all flex items-center justify-center gap-3">
               ...
               {hospitalToEdit ? t('update_partner_details') : t('register_partner_hospital')}
             </button>
