        ...
        <div className="p-6 lg:p-8 border-b border-slate-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button 
              ...
              className={`w-10 h-10 flex items-center justify-center rounded-xl transition-all ${showFilters ? 'bg-brand-navy text-white' : 'bg-slate-50 text-slate-400 hover:text-brand-navy'}`}
              title="تصفية المتقدمة"
            >
              <Filter size={18} />
            </button>
          </div>
        ...
        <div className="px-8 pb-6 animate-in slide-in-from-top-2 duration-300">
           <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-2">
                 <Filter size={14} className="text-brand-navy" />
                 <span className="text-[10px] font-black text-brand-navy uppercase tracking-widest">مصفي عرض البيانات (قيد التشغيل)</span>
              </div>
              <button 
            ...
            <table className="w-full text-right border-collapse">
              <thead>
                <tr className="bg-slate-50/50">
                  {columns.map((col, i) => (
                    <th key={i} className={`px-4 lg:px-6 py-5 text-[9px] lg:text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-100 whitespace-nowrap ${col.className || ''}`}>
                      {col.header}
                    </th>
                  ))}
                  {(onEdit || onDelete || onView) && (
                    <th className="px-4 lg:px-6 py-5 text-[9px] lg:text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-100 text-center whitespace-nowrap">الإجراءات</th>
                  )}
                </tr>
              </thead>
