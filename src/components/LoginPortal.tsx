...
export const LoginPortal: React.FC<LoginPortalProps> = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center p-6 relative overflow-hidden select-none">
      {/* Language Switcher in Login Portal */}
      ...
      >
        ...
        <div className="flex flex-col items-center mb-10">
          ...
          <h1 className="text-3xl font-black text-brand-navy tracking-tighter uppercase">{t('login_portal')}</h1>
           <p className="text-[10px] font-black uppercase text-brand-green tracking-[0.3em] mt-3 text-center">{t('tagline')}</p>
        </div>
        ...
        <div className="bg-white rounded-[3rem] shadow-2xl shadow-brand-navy/10 p-10 lg:p-12 border border-white relative overflow-hidden">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div>
              <label className="block text-[10px] font-black uppercase text-slate-400 mb-2.5 tracking-widest px-1">{t('username')}</label>
              <div className="relative group">
                <div className={`absolute inset-y-0 ${isRTL ? 'right-0 pr-4' : 'left-0 pl-4'} flex items-center pointer-events-none text-slate-300 group-focus-within:text-brand-navy transition-colors`}>
                  <User size={18} />
                </div>
                <input 
                  type="text"
                  ...
                  className={`w-full bg-slate-50 border-2 border-transparent rounded-2xl py-4 ${isRTL ? 'pr-12 pl-4' : 'pl-12 pr-4'} text-sm font-bold focus:bg-white focus:border-brand-navy/10 transition-all outline-none`}
            ...
            <div>
              <label className="block text-[10px] font-black uppercase text-slate-400 mb-2.5 tracking-widest px-1">{t('password')}</label>
              <div className="relative group">
                <div className={`absolute inset-y-0 ${isRTL ? 'right-0 pr-4' : 'left-0 pl-4'} flex items-center pointer-events-none text-slate-300 group-focus-within:text-brand-navy transition-colors`}>
                  <Lock size={18} />
                </div>
                <input 
                  ...
                  className={`w-full bg-slate-50 border-2 border-transparent rounded-2xl py-4 ${isRTL ? 'pr-12 pl-12' : 'pl-12 pr-12'} text-sm font-bold focus:bg-white focus:border-brand-navy/10 transition-all outline-none`}
                  placeholder="••••••••"
                ...
                <button 
                  ...
                  className={`absolute inset-y-0 ${isRTL ? 'left-0 pl-4' : 'right-0 pr-4'} flex items-center text-slate-300 hover:text-brand-navy transition-colors`}
                >
            ...
            >
              {isLoading ? (
                ...
                <>
                   <span>{t('login')}</span>
                   <ArrowLeft size={18} className={`${isRTL ? 'rotate-0 group-hover:-translate-x-1' : 'rotate-180 group-hover:translate-x-1'} transition-transform`} />
                </>
              )}
            </button>
          </form>
          ...
          <div className="mt-10 flex items-center justify-center gap-2 text-slate-300">
            <ShieldCheck size={14} />
            <span className="text-[9px] font-black uppercase tracking-widest">{t('authorized_only')}</span>
          </div>
        </div>
