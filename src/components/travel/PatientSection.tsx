...
export const PatientSection: React.FC = () => {
  ...
  const handleDelete = (id: string, name: string) => {
    if (confirm(`${t('confirm_archive')} "${name}"`)) {
  ...
  const columns = [
    { header: t('strategic_patient'), accessor: (p: Patient) => (
      <div className={`flex items-center gap-4 ${isRTL ? 'flex-row-reverse text-right' : 'text-left'}`}>
        <div className="w-12 h-12 bg-brand-navy/5 text-brand-navy rounded-2xl flex items-center justify-center font-black shadow-sm group-hover:bg-brand-navy group-hover:text-white transition-all shrink-0">
          {p.name.charAt(0)}
        </div>
        <div>
          <p className="font-black text-brand-navy tracking-tight leading-none">{p.name}</p>
          <div className="flex items-center gap-2 mt-1">
            <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest leading-none px-2 py-0.5 bg-slate-50 border border-slate-100 rounded-md tabular-nums">{p.id}</span>
            <span className="text-[9px] font-bold text-brand-cyan uppercase tracking-tighter shrink-0">{p.nationality || p.country}</span>
          </div>
        </div>
      </div>
    ), exportValue: (p: Patient) => p.name },
    { header: t('treatment_status'), accessor: (p: Patient) => (
      <div className={`flex flex-col ${isRTL ? 'text-right' : 'text-left'}`}>
        <span className="text-xs font-bold text-brand-navy mb-0.5 leading-none">{p.condition}</span>
        <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
           <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${p.assignedHospitalId ? 'bg-brand-green' : 'bg-slate-200'}`} />
           <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none">
             {p.assignedHospitalId ? t('hospital_assigned') : t('awaiting_analysis')}
           </span>
        </div>
      </div>
    ), exportValue: (p: Patient) => p.condition },
    { header: t('operational_progress'), accessor: (p: Patient) => (
      <span className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] shadow-sm border whitespace-nowrap ${
        ...
        {p.status === 'Active' ? t('status_processing') : 
         p.status === 'Confirmed' ? t('confirm') : 
         p.status === 'Inquiry' ? t('inquiries') : p.status}
      </span>
    ), exportValue: (p: Patient) => p.status },
    { header: t('last_update'), accessor: (p: Patient) => (
      <div className={`flex items-center gap-2 text-slate-400 tabular-nums ${isRTL ? 'flex-row-reverse' : ''}`}>
        <Calendar size={12} className="opacity-40" />
        <span className="text-[11px] font-bold italic">{new Date(p.lastContact).toLocaleDateString()}</span>
      </div>
    ...
    return (
      <div className={`space-y-8 animate-in fade-in slide-in-from-right-4 duration-500`}>
         <button 
          ...
          className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 hover:text-brand-navy transition-all mb-4 ${isRTL ? 'flex-row' : 'flex-row'}`}
         >
      ...
      <div className={`flex flex-col md:flex-row md:items-center justify-between gap-6 ${isRTL ? 'md:flex-row-reverse' : ''}`}>
        <div className={isRTL ? 'text-right' : 'text-left'}>
           <div className={`flex items-center gap-3 mb-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
              ...
              <h2 className="text-3xl font-black text-brand-navy tracking-tight uppercase leading-none">{t('patient_global_index')}</h2>
           </div>
           <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] leading-none mb-1">{t('patient_index_desc')}</p>
        </div>
