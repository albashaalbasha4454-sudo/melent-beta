  ...
  return (
    <div className={`space-y-10 animate-in fade-in duration-500 pb-20 ${isRTL ? 'text-right' : 'text-left'}`}>
       ...
       <div className={`flex flex-col md:flex-row md:items-center justify-between gap-6 ${isRTL ? 'md:flex-row-reverse' : ''}`}>
        <div className={isRTL ? 'text-right' : 'text-left'}>
           <div className={`flex items-center gap-3 mb-2 ${isRTL ? 'flex-row-reverse justify-start' : 'flex-row'}`}>
              ...
              <h2 className="text-3xl font-black text-brand-navy tracking-tight uppercase leading-none">{t('intelligence_portfolio')}</h2>
           </div>
           <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] leading-none mb-1">{t('intelligence_desc')}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
         {[
           { label: t('patient_conversion'), value: '68.4%', icon: Target, color: 'text-brand-cyan', bg: 'bg-brand-cyan/10' },
           { label: t('network_throughput'), value: '92.1%', icon: Globe, color: 'text-emerald-500', bg: 'bg-emerald-50' },
           { label: t('medical_arpu'), value: '$8,420', icon: DollarSign, color: 'text-brand-gold', bg: 'bg-brand-gold/10' },
           { label: t('strategic_growth'), value: '+24.5%', icon: TrendingUp, color: 'text-blue-500', bg: 'bg-blue-50' },
         ].map((stat, i) => (
           <div key={i} className={`bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm group hover:scale-105 transition-all ${isRTL ? 'text-right' : 'text-left'}`}>
              <div className={`w-12 h-12 ${stat.bg} ${stat.color} rounded-2xl flex items-center justify-center mb-6 ${isRTL ? 'mr-0 ml-auto' : 'ml-0 mr-auto'}`}>
                 <stat.icon size={24} />
              </div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 leading-none">{stat.label}</p>
              <p className="text-2xl font-black text-brand-navy tabular-nums leading-none">{stat.value}</p>
           </div>
         ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <div className="lg:col-span-2 bg-white rounded-[3.5rem] border border-slate-100 p-10 shadow-sm relative overflow-hidden">
            <div className={`flex flex-col md:flex-row md:items-start justify-between mb-8 gap-4 ${isRTL ? 'md:flex-row-reverse' : ''}`}>
               <div className={isRTL ? 'text-right' : 'text-left'}>
                  <h3 className="text-xl font-black text-brand-navy tracking-tight leading-none uppercase mb-2">{t('revenue_dynamics')}</h3>
                  <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest leading-none">{t('monthly_yield_vs_targets')}</p>
               </div>
               <div className={`flex gap-4 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
                  <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
                     <div className="w-3 h-3 rounded-full bg-brand-cyan" />
                     <span className="text-[9px] font-black uppercase text-slate-400 whitespace-nowrap">{t('actual_revenue')}</span>
                  </div>
                  <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
                     <div className="w-3 h-3 rounded-full bg-slate-100" />
                     <span className="text-[9px] font-black uppercase text-slate-400 whitespace-nowrap">{t('strategic_target')}</span>
                  </div>
               </div>
            </div>
            ...
            <ChartWrapper height={350}>
               <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={revenueData}>
                    ...
                    <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{fontSize: 10, fontWeight: 900, fill: '#94a3b8'}} reversed={isRTL} />
                    <YAxis axisLine={false} tickLine={false} orientation={isRTL ? 'right' : 'left'} tick={{fontSize: 10, fontWeight: 900, fill: '#94a3b8', tabularNums: true}} />
                    <Tooltip 
         ...
         <div className={`bg-brand-navy rounded-[3.5rem] p-10 text-white shadow-2xl shadow-brand-navy/30 relative flex flex-col justify-between ${isRTL ? 'text-right' : 'text-left'}`}>
            ...
            <div className="relative z-10">
               <h3 className="text-xl font-black tracking-tight mb-1 uppercase">{t('global_topology')}</h3>
               <p className="text-[10px] font-black text-brand-cyan uppercase tracking-widest opacity-60 leading-none">{t('lead_source_distribution')}</p>
            </div>
            ...
            <div className="relative z-10 space-y-4">
               {countryData.slice(0, 3).map((item, i) => (
                 <div key={i} className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse text-right' : 'flex-row text-left'}`}>
         ...
         <div className={`lg:col-span-3 bg-white rounded-[3.5rem] border border-slate-100 p-10 shadow-sm ${isRTL ? 'text-right' : 'text-left'}`}>
            <div className={`flex items-center justify-between mb-10 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
               <div>
                  <h3 className="text-xl font-black text-brand-navy tracking-tight uppercase leading-none mb-2">{t('clinical_vertical_performance')}</h3>
                  <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest leading-none">{t('medical_dept_analysis')}</p>
               </div>
               ...
               {categoryData.map((cat, i) => (
                 <div key={i} className="space-y-6">
                    ...
                    <div className="space-y-3">
                       <div className={`flex justify-between items-center bg-slate-50 p-4 rounded-2xl ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
                          <span className="text-[9px] font-black text-slate-400 uppercase tracking-tighter leading-none">{t('throughput')}</span>
                          <span className="text-xs font-black text-brand-navy whitespace-nowrap">{cat.patients} {t('cases')}</span>
                       </div>
                       <div className={`flex justify-between items-center bg-slate-50 p-4 rounded-2xl ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
                          <span className="text-[9px] font-black text-slate-400 uppercase tracking-tighter leading-none">{t('gross_yield')}</span>
                          <span className="text-xs font-black text-brand-navy whitespace-nowrap">${(cat.revenue / 1000).toFixed(0)}k</span>
                       </div>
                    </div>
                    <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden relative">
                       <div className={`h-full transition-all duration-1000 absolute top-0 ${isRTL ? 'right-0' : 'left-0'} bg-brand-navy`} style={{ width: `${(cat.patients / 240) * 100}%` }} />
                    </div>
