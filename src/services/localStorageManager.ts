...
export const LocalDB = {
  ...
  permanentlyDeleteFromRecycleBin: (binItemId: string) => {
    const bin = LocalDB.get(MELENT_KEYS.RECYCLE_BIN) || [];
    const binItem = bin.find((i: RecycleBinItem) => i.id === binItemId);
    const updatedBin = bin.filter((i: RecycleBinItem) => i.id !== binItemId);
    localStorage.setItem(MELENT_KEYS.RECYCLE_BIN, JSON.stringify(updatedBin));
    
    if (binItem) {
      LocalDB.logAction('USER_ACTION', 'DELETE', binItem.entityType, `Permanently deleted ${binItem.entityName}`);
    }
    return true;
  },
